...
if(isset($settings['text1']) && !empty($settings['text1']) && count($settings['text1'])): ?>
    <div class="pxl-swiper-slider pxl-text-carousel pxl-text-carousel2 <?php echo esc_attr($settings['style']); ?>" <?php if($drap !== false) : ?>data-cursor-drap="<?php echo esc_html('DRAG', 'nexros'); ?>"<?php endif; ?>>
        <div class="pxl-carousel-inner">
            <div <?php pxl_print_html($widget->get_render_attribute_string( 'carousel' )); ?>>
                <div class="pxl-swiper-wrapper">
                    <?php foreach ($settings['text1'] as $key => $value):
                        $link_key = $widget->get_repeater_setting_key( 'link1', 'value', $key );
                        $title1 = isset($value['title1']) ? $value['title1'] : '';
                        if ( ! empty( $value['link1']['url'] ) ) {
                            $widget->add_render_attribute( $link_key, 'href', $value['link1']['url'] );
                        ...
                        <div class="pxl-swiper-slide">
                            <div class="pxl-item--inner <?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms">
                                <a <?php echo implode( ' ', [ $link_attributes ] ); ?>><?php echo pxl_print_html($title1); ?></a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
