...
if (! class_exists ( 'Tnex_User_admin' )) {

	class Tnex_User_admin {  
		...
		function __construct() {
			...
			add_filter( 'plugin_action_links_' . tnexuser()->basename, array( $this, 'plugin_action_links' ) );
		}
		...
		function register_plugin_settings() {
			...
			register_setting('pxl-user-form-general-group', 'pxl_user_layout');
            register_setting('pxl-user-form-general-group', 'pxl_user_bg_color');
            register_setting('pxl-user-form-general-group', 'pxl_user_bg_img');
		...
		function plugin_action_links( $links ){
			...
			$action_links = array(
                 'settings' => '<a href="' . admin_url( 'users.php?page=pxl-user-form_admin' ) . '" title="' . esc_attr( esc_html__( 'View Tnex Users Settings', 'tnex-user' ) ) . '">' . esc_html__( 'Settings', 'tnex-user' ) . '</a>',
			);
		...
		function add_admin_page() {
			add_users_page ( esc_html__ ( 'Tnex Users', 'tnex-user' ), esc_html__ ( 'Tnex Users', 'tnex-user' ), 'manage_options', 'pxl-user-form_admin', array (
				$this,
				'add_admin_page_main'
			) );
		}
		...
		function add_admin_page_main() {
			...
			$tabs = array (
				'email' => esc_html__('Email Settings', 'tnex-user'),
			);

			$tabs = apply_filters('pxl-user-form/admin/tabs', $tabs);

			?>
			<h1><?php esc_html_e('Tnex User', 'tnex-user'); ?></h1>
			<p><?php esc_html_e('A wordpress user manager plugin.', 'tnex-user'); ?></p>
				...
				<form id="mainform" method="post" action="options.php">
					...
					<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
						<?php foreach ($tabs as $key => $tab): ?>
							<a href="<?php echo admin_url( 'users.php?page=pxl-user-form_admin&tab=' . $key ); ?>" class="nav-tab<?php echo ( $current_tab == $key ? ' nav-tab-active' : '' ) ; ?>"><?php echo esc_html($tab); ?></a>
						<?php endforeach; ?>
					</h2>
		...
		function add_admin_tab_content() {
			...
			$tab = apply_filters('pxl-user-form/inc/admin/tab/template', tnexuser()->plugin_dir . "admin/html_tab_$current_tab.php");

			if(!file_exists($tab)) return ;

			settings_fields( "pxl-user-form-$current_tab-group" );
			do_settings_sections( "pxl-user-form-$current_tab-group" );
