...
if (!class_exists('Nexros_Main')) { 
    class Nexros_Main extends Nexros_Base
    {
        ...
        public function get_theme_opt($setting = null, $default = false, $subset = false){
            ...
            if (empty(self::$options)) {
                ...
                $cache_key = 'nexros_theme_options_' . $this->get_option_name();
                self::$options = wp_cache_get($cache_key, 'nexros_theme');
                
                if (false === self::$options) {
                    self::$options = self::$instance->get_options();
                    // Cache for 1 hour
                    wp_cache_set($cache_key, self::$options, 'nexros_theme', 3600);
                }
            }
        ...
        public function set_options($setting, $value){
            ...
            $cache_key = 'nexros_theme_options_' . $this->get_option_name();
            wp_cache_delete($cache_key, 'nexros_theme');
        ...
        public function get_sidebar_args($args = []){
            ...
            $sidebar_style = $this->get_opt('sidebar_layout', 'style1');
...
function nexros() {
    return Nexros_Main::getInstance();
}
// Install
nexros(); 
