<?php
/**
 * The Nexros_Updater initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly
	
class Nexros_Updater {
	...
	function __construct( $args = array(), $strings = array() ) {
		...
		$args = wp_parse_args( $args, array(
			'remote_api_url' => 'http://api.tnexthemes.net/',
			'request_data' => array(),
	...
	function update_nag() {
		...
		if ( version_compare( $this->version, $api_response->new_version, '<' ) ) {
			...
			printf(
				wp_kses_post( esc_html__( '<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'nexros' ) ),
				$theme->get( 'Name' ),
